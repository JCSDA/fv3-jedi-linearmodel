...
use read_climate_nudge_data_nlm_mod, only:   &
 ...
 ! if no overrides turned on then set freq = 0
   if (freq > 0) then
       if ( .not.do_u .and. .not.do_v .and. .not.do_t .and. &
            ...
            call error_mesg ('fv_climate_nudge_nlm_mod', 'no variables specified '//&
                             'for override', WARNING)
       endif
   else
       if ( do_u .or. do_v .or. do_t .or.  do_q .or. do_ps ) then
            call error_mesg ('fv_climate_nudge_nlm_mod', 'variables specified '//&
...
! get the size of the global data
  call error_mesg ('fv_climate_nudge_nlm_mod', 'initializing nudging', NOTE)
...
logical :: virtual_temp_obs = .false.
   ...
   if (.not.module_is_initialized) then
       call error_mesg ('fv_climate_nudge_nlm_mod', 'module not initialized', FATAL)
   endif
 ...
 !     call error_mesg ('fv_climate_nudge_nlm_mod', 'only one tracer (sphum) allowed', FATAL)
 ! endif
...
!//////////////////////////////////////////////////////
  ...
  ! get the time indices needed
    ...
    if(err_msg /= '') then
       call error_mesg('fv_climate_nudge_nlm_mod',trim(err_msg), FATAL)
    endif
 ...
 ! resolution must match
   if (State1%is /= State2%is .or. State1%ie /= State2%ie .or. &
       ...
       State1%npz /= State2%npz) then
            call error_mesg ('fv_climate_nudge_nlm_mod', 'invalid var_state assignment: '// &
...
end module fv_climate_nudge_nlm_mod
